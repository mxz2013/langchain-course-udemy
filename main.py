...
"""
    ...
    # created a prompt template that takes the information as input and formats it according to the specified template

    # llm = ChatOpenAI(model="gpt-5", temperature=0.0)
    llm = ChatOllama(model="gpt-oss:latest", temperature=0.0)
    # temperature 0-0.3 is more deterministic, 0.7-1.0 is more creative

    chain = (
        summary_template | llm
